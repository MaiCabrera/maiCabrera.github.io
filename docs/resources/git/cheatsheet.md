...
| Command | Description |
|---------|-------------|
| `git config  user.name "your_username"` | Author |
| `git config  user.email "your_email"` | Author email |
...
| Command | Description |
|---------|-------------|
| `git config --global --unset <config-key>` | Unset global config  > ex: git config --global --unset user.name |
| `git config --unset <config-key>` | Unset local config  > ex: git config --unset user.name|
...
| `git rebase -i HEAD~n` | Opens list of the last n commits,> replace _"pick"_ by _"reword"_ to alter the commit message of those commits. |
